...
class EditImageRepositoryImpl(private val context: Context) : EditImageRepository {
    ...
    override suspend fun getImageFilters(image: Bitmap): List<ImageFilter> {
        ...
        GPUImageColorMatrixFilter(
            0.9f,
            floatArrayOf(
                1.025f, 0.1f, 0.1f, 0.02f,
                0.0f, 0.9f, 0.1f, 0.1f,
                0.05f, 0.1f, 0.9f, 0.1f,
                1.0f, 0.0f, 0.0f, 1.0f,
            )
        ).also { filter ->
            gpuImage.setFilter(filter)
    ...
    override suspend fun saveImageFilteredImage(filteredBimap: Bitmap): Uri? {
        return try {
